		...
		stage('Sonarqube') {
			...
			steps{
				withSonarQubeEnv(installationName:'Asg Wintermute Server') {
				   git url: 'https://github.com/ASG-BPM/cosmonaut-nomina',branch:'main',credentialsId: 'winter_user'    
				   sh "git checkout $tag"
    		       sh 'gradle sonarqube --stacktrace'
				}
			}
		}
		    ...
		    steps{
		        script{
    		        docker.withRegistry('https://gcr.io', 'gcr:cosmonaut-registry-sa') {
                        docker.image("gcr.io/cosmonaut-299500/cosmonaut/nomina:$tag").push(tag)
                    }   
		        }
		    }
	    }
			...
			steps{
			     script{
    			     withCredentials([file(credentialsId: 'cosmonaut-k8s-sa', variable: 'serviceaccount')]) {
        		       ...
        		       sh "kubectl get pods"
        		       sh "sed -i 's/\$TAG/$tag/' src/jenkins/cosmonaut_nomina_deployment.yml"
        		       sh "cat src/jenkins/cosmonaut_nomina_deployment.yml | kubectl apply -f -"
    		         }
			     }
			}
		}
