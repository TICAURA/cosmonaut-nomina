podTemplate(containers:[
  ...
  node(POD_LABEL) {
    stage("Compilacion") {
        container('gradle'){
            script{
			   git url: 'https://github.com/ASG-BPM/cosmonaut-nomina',branch:'main',credentialsId: 'winter_user'
     ...
     stage("Push de imagen") {
        container('docker-registry'){
            script{
				sh "/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --cache=true --destination=gcr.io/cosmonaut-uat/cosmonaut/nomina:$tag"
			}
        }
     }
        ...
        container('container-registry'){
            script{
		      ...
		      sh "kubectl  --insecure-skip-tls-verify get pods"
		       sh "sed -i 's/\$TAG/$tag/' src/jenkins/cosmonaut-nomina_deployment-qa.yml"
		       sh "cat src/jenkins/cosmonaut-nomina_deployment-qa.yml | kubectl --insecure-skip-tls-verify  apply -f -"
